...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (session_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (session_hotpatch):",
          "|", run_config_as_str, "|")
...
class BaseSession(SessionInterface):
  ...
  def __init__(self, target='', graph=None, config=None):
    ...
    if config is None:
      hprint(f"[BaseSession.__init__] No config exists so creating a new config (this could trigger new context creation)")
    ...
    if DISPLAY_SESSION_CONFIG:
      print_session_config("BaseSession.__init__ right before we try to create the session with tf_session.TF_NewSessionOptions", config)
    opts = tf_session.TF_NewSessionOptions(target=self._target, config=config)
    hprint(f"BaseSession.__init__ opts that are going into TF_NewSessionRef (there's a decent chance this doesn't have a valid str representation): {str(opts)}")
    try:
...
class Session(BaseSession):
  ...
  def __init__(self, target='', graph=None, config=None):
    ...
    """
    hprint("[Session.__init__] Entered __init__")
...
class InteractiveSession(BaseSession):
  ...
  def __init__(self, target='', graph=None, config=None):
    ...
    """
    hprint("[InteractiveSession.__init__] Entered __init__")
    ...
    if not config:
      ...
      hprint("[InteractiveSession.__init__] Config was not provided so setting up TF default config")

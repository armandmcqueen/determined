...
HARDCODE_NCHW_CHECK = read_envvar_bool("DET_DEBUG_HARDCODE_NCHW_CHECK")

print("HOTPATCH", "keras_backend.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")
print("HOTPATCH", "keras_backend.py", f"HARDCODE_NCHW_CHECK = {HARDCODE_NCHW_CHECK}")


def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "keras_backend", "custom logs:", s)
...
def _has_nchw_support():
  ...
  if HARDCODE_NCHW_CHECK:
    hprint("Keras using hardcoded gpu list for NCHW check")
    gpus_available = True

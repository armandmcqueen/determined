...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (framework_device_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (framework_device_hotpatch):",
          "|", run_config_as_str, "|")
...
print("HOTPATCH", "framework_device_hotpatch.py", f"DISPLAY_SESSION_CONFIG = {DISPLAY_SESSION_CONFIG}")
print("HOTPATCH", "framework_device_hotpatch.py", f"DISPLAY_RUN_CONFIG = {DISPLAY_RUN_CONFIG}")
print("HOTPATCH", "framework_device_hotpatch.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")
print("HOTPATCH", "framework_device_hotpatch.py", f"LOG_CALL_STACK = {LOG_CALL_STACK}")


def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "framework_device_hotpatch", "custom logs:", s)
...
class MergeDevice(object):
  ...
  def __init__(self, spec):
    hprint("Entering MergeDevice.__init__")
    if LOG_CALL_STACK:
        frame = sys._getframe(1)
        hprint(f"[MergeDevice.__init__ call stack inspection] Function calling DeviceSpecV2: {frame.f_code.co_name}, {frame.f_code.co_filename}")

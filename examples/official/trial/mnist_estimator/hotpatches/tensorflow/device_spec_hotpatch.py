...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (device_spec_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (device_spec_hotpatch):",
          "|", run_config_as_str, "|")
...
print("HOTPATCH", "device_spec_hotpatch.py", f"DISPLAY_SESSION_CONFIG = {DISPLAY_SESSION_CONFIG}")
print("HOTPATCH", "device_spec_hotpatch.py", f"DISPLAY_RUN_CONFIG = {DISPLAY_RUN_CONFIG}")
print("HOTPATCH", "device_spec_hotpatch.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")
print("HOTPATCH", "device_spec_hotpatch.py", f"LOG_CALL_STACK = {LOG_CALL_STACK}")


def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "device_spec_hotpatch", "custom logs:", s)
...
class DeviceSpecV2(object):
  ...
  * Device index: The device index.
  ...
  def __init__(self, job=None, replica=None, task=None, device_type=None,
    ...
    """
    hprint(f"[DeviceSpecV2.__init__] Entered init and creating new device specification")
    if LOG_CALL_STACK:
      frame = sys._getframe(1)
      hprint(f"[DeviceSpecV2.__init__ call stack inspection] Function calling DeviceSpecV2: {frame.f_code.co_name}, {frame.f_code.co_filename}")
    ...
    hprint(f"[DeviceSpecV2.__init__] Parsed args to display {self._as_string}: job={self._job}, replica={self._replica}, task={self._task}, device_type={self._device_type}, device_index={self._device_index}")
  ...
  def from_string(cls, spec):
    ...
    if LOG_CALL_STACK:
      frame = sys._getframe(1)
      hprint(f"[DeviceSpecV2.from_string() call stack inspection] Function calling DeviceSpecV2.from_string(): {frame.f_code.co_name}, {frame.f_code.co_filename}")

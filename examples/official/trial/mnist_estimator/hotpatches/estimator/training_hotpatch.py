...
OVERWRITE_TF_CONFIG_ENVVAR_NAME = read_envvar_bool("DET_DEBUG_OVERWRITE_TF_CONFIG_ENVVAR_NAME")
VERBOSE_CUSTOM_LOGGING = read_envvar_bool("DET_DEBUG_VERBOSE_CUSTOM_LOGGING")

print("HOTPATCH", "training_hotpatch.py", f"DISPLAY_SESSION_CONFIG = {DISPLAY_SESSION_CONFIG}")
print("HOTPATCH", "training_hotpatch.py", f"DISPLAY_RUN_CONFIG = {DISPLAY_RUN_CONFIG}")
print("HOTPATCH", "training_hotpatch.py", f"OVERWRITE_TF_CONFIG_ENVVAR_NAME = {OVERWRITE_TF_CONFIG_ENVVAR_NAME}")
print("HOTPATCH", "training_hotpatch.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")
...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (training_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (training_hotpatch):",
          "|", run_config_as_str, "|")

def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "training_hotpatch", "custom logs:", s)
...
if OVERWRITE_TF_CONFIG_ENVVAR_NAME:
    print("HOTPATCH", "Changing name of TF_CONFIG environment variable used by Estimator")
    _TF_CONFIG_ENV = 'NOT_TF_CONFIG_AND_UNLIKELY_TO_HAVE_NAMING_COLLISION'
...
def train_and_evaluate(estimator, train_spec, eval_spec):
  ...
  """
  hprint("Entering train_and_evaluate")
  ...
  if DISPLAY_SESSION_CONFIG:
    print_session_config("Beginning of Estimator.train_and_evaluate", estimator._session_config)
  if DISPLAY_RUN_CONFIG:
    print_run_config("Beginning of Estimator.train_and_evaluate", estimator._config)
  if distribute_coordinator_training.should_run_distribute_coordinator(config):
    hprint("[train_and_evaluate] Using distribute_coordinator (LOGS DO NOT TRACK BEYOND THIS)")
  ...
  hprint("[train_and_evaluate] executor.run()")
  return executor.run()
...
class _TrainingExecutor(object):
  ...
  def run(self):
    ...
    """
    hprint("[executor.run] Entering run")
    config = self._estimator.config

    if (not config.cluster_spec and
        config.task_type != run_config_lib.TaskType.EVALUATOR):
      hprint("[executor.run] Running training and evaluation locally")
      logging.info('Running training and evaluation locally (non-distributed).')
      return self.run_local()

    hprint("[executor.run] Not running training and evaluation locally. TensorFlow thinks it is supposed to be handling distributed training.")
  ...
  def run_local(self):
    ...
    hprint("[executor.run_local] Entering run_local")
    _assert_eval_spec(self._eval_spec)
    ...
    hprint("[executor.run_local] Entering estimator.train")

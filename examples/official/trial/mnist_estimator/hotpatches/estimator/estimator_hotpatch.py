...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (estimator_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (estimator_hotpatch):",
          "|", run_config_as_str, "|")
...
print("HOTPATCH", "estimator_hotpatch.py", f"DISPLAY_SESSION_CONFIG = {DISPLAY_SESSION_CONFIG}")
print("HOTPATCH", "estimator_hotpatch.py", f"DISPLAY_RUN_CONFIG = {DISPLAY_RUN_CONFIG}")
print("HOTPATCH", "estimator_hotpatch.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")


def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "estimator_hotpatch", "custom logs:", s)
...
class Estimator(object):
  ...
  def train(self,
    ...
    """
    hprint("[Estimator.train] Entering train")
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Beginning of Estimator.train", self._config.session_config)
    if DISPLAY_RUN_CONFIG:
      print_run_config("Beginning of Estimator.train", self._config)
    ...
    with context.graph_mode():
      ...
      hprint("[Estimator.train] About to enter _train_model")
      loss = self._train_model(input_fn, hooks, saving_listeners)
      hprint("[Estimator.train] Exited _train_model")
  ...
  def _call_model_fn(self, features, labels, mode, config):
    ...
    """
    if DISPLAY_RUN_CONFIG:
      print_run_config("Estimator._call_model_fn() start", config)
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Estimator._call_model_fn() start", config.session_config)
    ...
    if VERBOSE_CUSTOM_LOGGING:
      hprint(f"[Estimator._call_model_fn] Right before calling _call_model_fn. Model fn will be called with these params: features={features}, kwargs={kwargs}")
    if DISPLAY_RUN_CONFIG:
      print_run_config("Estimator._call_model_fn() right before calling _call_model_fn", config)
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Estimator._call_model_fn() right before calling _call_model_fn", config.session_config)
    ...
    if DISPLAY_RUN_CONFIG:
      print_run_config("Estimator._call_model_fn() after model_fn called", config)
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Estimator._call_model_fn() after model_fn called", config.session_config)
  ...
  def _train_model(self, input_fn, hooks, saving_listeners):
    hprint("[Estimator._train_model] Entered _train_model")
    if self._train_distribution:
      hprint("[Estimator._train_model] self._train_distribution was true so about to enter Estimator._train_model_distributed")
      return self._train_model_distributed(input_fn, hooks, saving_listeners)
    else:
      hprint("[Estimator._train_model] self._train_distribution was false so about to enter Estimator._train_model_default")
  ...
  def _train_model_default(self, input_fn, hooks, saving_listeners):
    ...
    Returns:
      Loss from training
    """
    hprint("[Estimator._train_model_default] Entered _train_model_default")
    worker_hooks = []
    hprint(f"[Estimator._train_model_default] [1] Looking at device_fn: {self._device_fn}")
    with ops.Graph().as_default() as g, g.device(self._device_fn):
      hprint(f"[Estimator._train_model_default] [2] In context manager (new default graph and g.device(self._device_fn)")
      random_seed.set_random_seed(self._config.tf_random_seed)
      hprint(f"[Estimator._train_model_default] [3] After setting random seed")
      global_step_tensor = self._create_and_assert_global_step(g)
      hprint(f"[Estimator._train_model_default] [4] After establishing global step")

      # if VERBOSE_CUSTOM_LOGGING:
      #   hprint("Inspecting visible vs physical vs logical devices")
      #   visible_devices = config.get_visible_devices()
      #   hprint(f"Visible devices: {visible_devices}")
      #   physical_devices = config.list_physical_devices()
      #   hprint(f"Physical devices: {physical_devices}")
      #   logical_devices = config.list_logical_devices()
      #   hprint(f"Logical devices: {logical_devices}")

      # if VERBOSE_CUSTOM_LOGGING:
      #   hprint("Checking to see the devices available to the local process (this might be useless)")
      #   list_of_device_attributes_when_no_session_config = device_lib.list_local_devices(session_config=None)
      #   hprint(f"Local devices when no session config {list_of_device_attributes_when_no_session_config}")
      #   list_of_device_attributes_when_actual_session_config = device_lib.list_local_devices(session_config=self._session_config)
      #   hprint(f"Local devices when actual session config {list_of_device_attributes_when_actual_session_config}")

      if VERBOSE_CUSTOM_LOGGING:
        # TODO:  Inspect the current session and if it is setting bad default by ignoring the estimator spec
        pass
      ...
      if global_step_tensor is not None:
        hprint(f"[Estimator._train_model_default] [5] In global step if statement")
        training_util._get_or_create_global_step_read(g)  # pylint: disable=protected-access

      features, labels, input_hooks = (
          self._get_features_and_labels_from_input_fn(
              input_fn, ModeKeys.TRAIN))
      hprint(f"[Estimator._train_model_default] [6] After _get_features_and_labels_from_input_fn (remember: step [5] is inside a conditional and might not appear in logs)")
      worker_hooks.extend(input_hooks)
      hprint(f"[Estimator._train_model_default] [7] After adding input hooks to worker_hooks.")
      hprint(f"[Estimator._train_model_default] [7.1] About to call model_fn")
      estimator_spec = self._call_model_fn(
          features, labels, ModeKeys.TRAIN, self.config)
      hprint(f"[Estimator._train_model_default] [8] After calling model_fn")
      global_step_tensor = training_util.get_global_step(g)
      hprint(f"[Estimator._train_model_default] [9] After retrieving global step")
      if DISPLAY_SESSION_CONFIG:
        print_session_config("_train_model_default", self._config.session_config)
      if DISPLAY_RUN_CONFIG:
        print_run_config("_train_model_default", self._config)
      hprint(f"[Estimator._train_model_default] [10] Entering _train_with_estimator_spec")
  ...
  def _train_model_distributed(self, input_fn, hooks, saving_listeners):
    ...
    """
    # pylint: disable=protected-access
    hprint("[Estimator._train_model_distributed] Entered _train_model_distributed")
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Estimator._train_model_distributed", self._config.session_config)
    if DISPLAY_RUN_CONFIG:
      print_run_config("Estimator._train_model_distributed", self._config)
    if (hasattr(self._config, '_distribute_coordinator_mode') and
      ...
      hprint("[Estimator._train_model_distributed] Took path 1 (seems to be using distribute_coordinator?). Logs end here as we enter core Tensorflow")
      ...
      hprint("[Estimator._train_model_distributed] Took path 2. About to enter actual estimator distributed training code. Logs end here (but they could go deeper)")
  ...
  def _train_with_estimator_spec_distributed(self, estimator_spec, worker_hooks,
    ...
    hprint("[Estimator._train_with_estimator_spec_distributed] Entered _train_with_estimator_spec_distributed")
    if DISPLAY_SESSION_CONFIG:
      print_session_config("_train_with_estimator_spec_distributed", self._session_config)
    if DISPLAY_RUN_CONFIG:
      print_run_config("_train_with_estimator_spec_distributed", self._config)
    if saving_listener:
    ...
    hprint("[Estimator._train_with_estimator_spec_distributed] Exiting")
    return loss

  def _train_with_estimator_spec(self, estimator_spec, worker_hooks, hooks,
                                 global_step_tensor, saving_listeners):
    """Train a model with the given Estimator Spec."""
    hprint("[Estimator._train_with_estimator_spec] Entered _train_with_estimator_spec")
    if DISPLAY_SESSION_CONFIG:
      print_session_config("Beginning of _train_with_estimator_spec", self._config.session_config)
    if DISPLAY_RUN_CONFIG:
      print_run_config("Beginning of _train_with_estimator_spec", self._config)
    if self._warm_start_settings:
      hprint("[Estimator._train_with_estimator_spec] Doing warm start")
    ...
    if (self._config.cluster_spec and type(
      ...
      hprint("[Estimator._train_with_estimator_spec] Cluster spec exists and distribution strategy is one of the valid ones so about to enter _train_with_estimator_spec_distributed")
    ...
    if (self._config.cluster_spec and self._config.cluster_spec.jobs and
      ...
      hprint("[Estimator._train_with_estimator_spec] Location 1 (adding hooks based on cluster layout?)")
    ...
    with training.MonitoredTrainingSession(
      ...
      hprint("[Estimator._train_with_estimator_spec] Entered MonitoredTrainingSession context")
      if DISPLAY_SESSION_CONFIG:
        print_session_config("MonitoredTrainingSession before sess.run", self._config.session_config)
      if DISPLAY_RUN_CONFIG:
        print_run_config("MonitoredTrainingSession before sess.run", self._config)
      loss = None
      any_step_done = False
      while not mon_sess.should_stop():
        _, loss = mon_sess.run([estimator_spec.train_op, estimator_spec.loss])
    ...
    if not any_step_done:
      logging.warning('Training with estimator made no steps. '
                      'Perhaps input is empty or misspecified.')
    hprint("[Estimator._train_with_estimator_spec] Exiting")
    return loss

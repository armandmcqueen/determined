...
def print_session_config(log_location_id, session_config):
    ...
    print("HOTPATCH", f"session_config at {log_location_id} (_estimator_trial_hotpatch):",
          "|", config_proto_as_str, "|")

def print_run_config(log_location_id, run_config):
    run_config_as_str = str(vars(run_config))
    print("HOTPATCH", f"run_config at {log_location_id} (_estimator_trial_hotpatch):",
          "|", run_config_as_str, "|")
...
BREAK_VISIBLE_DEVICE_LIST = read_envvar_bool("DET_DEBUG_BREAK_VISIBLE_DEVICE_LIST")
CHECK_DEFAULT_TF_SESSION = read_envvar_bool("DET_DEBUG_CHECK_DEFAULT_TF_SESSION")
VERBOSE_CUSTOM_LOGGING = read_envvar_bool("DET_DEBUG_VERBOSE_CUSTOM_LOGGING")



print("HOTPATCH", "_estimator_trial.py", f"DISPLAY_SESSION_CONFIG = {DISPLAY_SESSION_CONFIG}")
print("HOTPATCH", "_estimator_trial.py", f"DISPLAY_RUN_CONFIG = {DISPLAY_RUN_CONFIG}")
print("HOTPATCH", "_estimator_trial.py", f"BREAK_VISIBLE_DEVICE_LIST = {BREAK_VISIBLE_DEVICE_LIST}")
print("HOTPATCH", "_estimator_trial.py", f"CHECK_DEFAULT_TF_SESSION = {CHECK_DEFAULT_TF_SESSION}")
print("HOTPATCH", "_estimator_trial.py", f"VERBOSE_CUSTOM_LOGGING = {VERBOSE_CUSTOM_LOGGING}")


def hprint(s):
    if VERBOSE_CUSTOM_LOGGING:
        print("HOTPATCH", "_estimator_trial_hotpatch", "custom logs:", s)


if CHECK_DEFAULT_TF_SESSION:
    from tensorflow.python.framework import ops as tf_ops

def check_default_tf_session(print_location):
    hprint(f"{print_location}: Checking default tf_session stack")
    num_defaults_sessions_stacked = len(tf_ops._default_session_stack.stack)
    hprint(f"{print_location}: Length of default tf session stack: {num_defaults_sessions_stacked}")

def inspect_estimator_settings(estimator: tf.estimator.Estimator):
    # device_fn = estimator._device_fn
    # print("HOTPATCH", "Estimator device_fn", str(device_fn))

    # TODO: Not sure train distribution will convert to string well
    # train_distributed = estimator._train_distribution
    # print("HOTPATCH", "Estimator train_distribute", str(train_distributed))

    # https://github.com/tensorflow/tensorflow/blob/r1.15/tensorflow/core/protobuf/config.proto
    s = estimator._session_config
    # print("HOTPATCH", "Estimator sess_config.per_process_gpu_memory_fraction", str(s.gpu_options.per_process_gpu_memory_fraction))
    # print("HOTPATCH", "Estimator sess_config.allow_growth", str(s.gpu_options.allow_growth))
    # print("HOTPATCH", "Estimator sess_config.allocator_type", str(s.gpu_options.allocator_type))
    # print("HOTPATCH", "Estimator sess_config.visible_device_list", str(s.gpu_options.visible_device_list))
    # print("HOTPATCH", "Estimator sess_config.virtual_devices", str(
    #     s.gpu_options.experimental.virtual_devices))  # TODO: This is a repeated field. We may need to iterate and inspect virtual_device.memory_limit_mb.
    #
    # print("HOTPATCH", "Estimator sess_config.device_count", str(
    #     s.device_count))  # map<string, int32> device_count = 1; TODO: May need to unroll this to inspect it
    # print("HOTPATCH", "Estimator sess_config.device_filters",
    #       str(s.device_filters))  # repeated string device_filters = 4; TODO: maybe unroll
    # print("HOTPATCH", "Estimator sess_config.log_device_placement", str(s.log_device_placement))
    # print("HOTPATCH", "Estimator sess_config.experimental.collective_nccl", str(s.experimental.collective_nccl))
    # print("HOTPATCH", "Estimator sess_config.experimental.share_session_state_in_clusterspec_propagation",
    #       str(s.experimental.share_session_state_in_clusterspec_propagation))
    # print("HOTPATCH", "Estimator sess_config.experimental.share_cluster_devices_in_session",
    #       str(s.experimental.share_cluster_devices_in_session))

    # print("HOTPATCH", "Estimator sess_config.cluster_def",
    #       str(s.cluster_def))  # This is a ClusterDef. TODO: Need to Print out specific subfields

    # https://github.com/tensorflow/tensorflow/blob/r1.15/tensorflow/core/protobuf/cluster.proto
    # print("HOTPATCH", "checking if cluster def exists:", bool(s.cluster_def))
    # print("HOTPATCH", "checking cluster def type:", type(s.cluster_def))
    # print("HOTPATCH", "maybe about to inspect cluster def")
    # for i, job in enumerate(s.cluster_def.job):
    #     print("HOTPATCH", "ClusterDef job name", i, job.name)
    #     print("HOTPATCH", "ClusterDef task map", i, job.tasks)

    # print("HOTPATCH", "maybe about to inspect cluster def using proto.text_format")
    # cluster_def_as_str = text_format.MessageToString(s.cluster_def)
    # print(type(cluster_def_as_str))
    # print(cluster_def_as_str)
    # print("HOTPATCH", "text_format.MessageToString(s.cluster_def)", cluster_def_as_str)

    config_proto_as_str = text_format.MessageToString(s, as_one_line=True)
    print("HOTPATCH", "session_config at tf.estimator_train_and_evaluate entrypoint (harness):",
          "|", config_proto_as_str, "|")

    # print("HOTPATCH", "maybe about to inspect virtual devices")
    # for i, virtual_device in enumerate(s.gpu_options.experimental.virtual_devices):
    #     print("HOTPATCH", "virtual device iterator", i, virtual_device.memory_limit_mb)  # memory_limit_mb is also a repeated field so we may need to unroll more - https://github.com/tensorflow/tensorflow/blob/r1.15/tensorflow/core/protobuf/config.proto#L103

    print("HOTPATCH", "env:", os.environ)
...
class EstimatorTrialController(det.LoopTrialController):
    ...
    def _init_model(self) -> None:
        ...
        if DISPLAY_SESSION_CONFIG:
            print_session_config("[_init_model] Beginning (user's original estimator)", self.estimator._session_config)
        if DISPLAY_RUN_CONFIG:
            print_run_config("[_init_model] Beginning (user's original estimator)", self.estimator._config)
        if CHECK_DEFAULT_TF_SESSION:
            check_default_tf_session("[_init_model] Beginning (user's original estimator)")
        ...
        if DISPLAY_SESSION_CONFIG:
            print_session_config("[_init_model] After wrapping user's model", self.estimator._session_config)
        if DISPLAY_RUN_CONFIG:
            print_run_config("[_init_model] After wrapping user's model", self.estimator._config)
        if CHECK_DEFAULT_TF_SESSION:
            check_default_tf_session("[_init_model] After wrapping user's model")
    ...
    def _init_run_config(self, config: tf.estimator.RunConfig) -> tf.estimator.RunConfig:
        ...
        print("HOTPATCH", "About to replace RunConfig fields - confirming train_distribute value:", str(train_distribute))
        ...
        if DISPLAY_SESSION_CONFIG:
            config_proto_as_str = text_format.MessageToString(config._session_config, as_one_line=True)
            print("HOTPATCH", "session_config at RunConfig creation (harness):",
                  "|", config_proto_as_str, "|")
        if DISPLAY_RUN_CONFIG:
            print_run_config("session_config at RunConfig creation (harness):", config)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if DISPLAY_RUN_CONFIG:
                print_run_config("Right before train_and_evaluate (harness)", self.estimator._config)

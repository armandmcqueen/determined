...
DROP_USER_RUN_CONFIG = False
DELAY_RANK_START = False
DELAY_TRIAL_CREATION = False

BREAK_VISIBLE_DEVICE_LIST = False
LOG_DEVICE_PLACEMENT = False
DISPLAY_SESSION_CONFIG = True
...
class DeterminedControlHook(tf.estimator.SessionRunHook):  # type: ignore
    ...
    def begin(self) -> None:
        ...
        # in the RunConfig.
        print("CONFIRMING HOTPATCH LOADED")
...
class EstimatorTrialController(det.LoopTrialController):
    ...
    def pre_execute_hook(env: det.EnvContext, hvd_config: horovod.HorovodContext) -> None:
        ...
        if hvd_config.use:
            ...
            if USE_CUDA_VISIBLE_DEVICES:
                print("Setting CUDA visisble device")
                os.environ["CUDA_VISIBLE_DEVICES"] = str(hvd.local_rank())

            if DELAY_TRIAL_CREATION:
                print(f"rank {hvd.local_rank()} sleeping before creating trial ...")
                import time
                time.sleep(hvd.local_rank() * 10)
                print(f"rank {hvd.local_rank()} now creating trial ...")
    ...
    def _init_run_config(self, config: tf.estimator.RunConfig) -> tf.estimator.RunConfig:
        if DROP_USER_RUN_CONFIG:
            config = tf.estimator.RunConfig()

        logging.warning(f"Initializing RunConfig. Got RunConfig: {config.__dict__}.")
        ...
        if self.hvd_config.use:
            ...
            if not USE_CUDA_VISIBLE_DEVICES:
                if BREAK_VISIBLE_DEVICE_LIST:
                    ...
                    print("Attempting to set visible devices to all GPUs:", visible_device_list)
                    session_config.gpu_options.visible_device_list = str(
                        visible_device_list
                    )
                else:
                    visible_device_list = self.env.slot_ids[horovod.hvd.local_rank()]
                    print("Using unbroken visible devices:", visible_device_list)
                    session_config.gpu_options.visible_device_list = str(
                        visible_device_list
                    )
        ...
        logging.warning(f"Going to use RunConfig with args: {config.__dict__}.")
        return config

    def run(self) -> None:
        # logging.info(f"Physical devices list: {tf.config.experimental.list_physical_devices('GPU')}")
        # logging.info(f"Physical devices get: {tf.config.experimental.get_visible_devices('GPU')}")

        if DELAY_RANK_START:
            import time
            print(f"rank {horovod.hvd.local_rank()} sleeping ...")
            time.sleep(horovod.hvd.local_rank() * 10)
            print(f"rank {horovod.hvd.local_rank()} starting estaimtor run ...")
        ...
        def inspect_estimator_settings(estimator: tf.estimator.Estimator):
            ...
            print("HOTPATCH", "Estimator sess_config.virtual_devices", str(s.gpu_options.experimental.virtual_devices)) # TODO: This is a repeated field. We may need to iterate and inspect virtual_device.memory_limit_mb.


            print("HOTPATCH", "Estimator sess_config.device_count", str(s.device_count))  # map<string, int32> device_count = 1; TODO: May need to unroll this to inspect it
            print("HOTPATCH", "Estimator sess_config.device_filters", str(s.device_filters))  # repeated string device_filters = 4; TODO: maybe unroll
            ...
            print("HOTPATCH", "Estimator sess_config.experimental.share_session_state_in_clusterspec_propagation", str(s.experimental.share_session_state_in_clusterspec_propagation))
            print("HOTPATCH", "Estimator sess_config.experimental.share_cluster_devices_in_session", str(s.experimental.share_cluster_devices_in_session))

            print("HOTPATCH", "Estimator sess_config.cluster_def", str(s.cluster_def))  # This is a ClusterDef. TODO: Need to Print out specific subfields
            ...
            print("HOTPATCH", "maybe about to inspect ConfigProto using proto.text_format")
            config_proto_as_str = text_format.MessageToString(s)
            print(type(config_proto_as_str))
            print(config_proto_as_str)
            print("HOTPATCH", "text_format.MessageToString(s)", config_proto_as_str)
            ...
            HOTPATCH Estimator device_fn None
            HOTPATCH Estimator train_distribute None
            HOTPATCH Estimator sess_config.per_process_gpu_memory_fraction 0.0
            HOTPATCH Estimator sess_config.allow_growth True
            HOTPATCH Estimator sess_config.allocator_type
            HOTPATCH Estimator sess_config.visible_device_list 7
            HOTPATCH Estimator sess_config.virtual_devices []
            HOTPATCH Estimator sess_config.device_count {}
            HOTPATCH Estimator sess_config.device_filters []
            HOTPATCH Estimator sess_config.log_device_placement False
            HOTPATCH Estimator sess_config.cluster_def
            HOTPATCH Estimator sess_config.experimental.collective_nccl False
            HOTPATCH Estimator sess_config.experimental.share_session_state_in_clusterspec_propagation False
            HOTPATCH Estimator sess_config.experimental.share_cluster_devices_in_session False
            """
